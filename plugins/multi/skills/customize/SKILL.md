...
→ Create `plugins/codex/commands/explore.md` (copy `plugins/codex/commands/review.md` and run `task --cli codex --role explore --read-only` directly — no subagent, since a read-only Q&A bridge needs no framing).
...
2. Step 1: `ls $REPO/plugins/multi/agents/` → see the read-only forwarders that exist, none for review (`/codex:review` dispatches straight from its command file). `cat .../adapters/cursor.mjs` → note `review`/`reviewer` is **not** in `READ_ONLY_ROLES`, so a `cursor` review role would default to write mode.
...
6. Command: create `plugins/cursor/commands/review.md` — copy `plugins/codex/commands/review.md` and change the Bash call to `task --cli cursor --role reviewer --read-only`. No subagent: a pure bridge runs from the main loop.
7. Optionally disable Codex's reviewer (delete/underscore `plugins/codex/commands/review.md`) if Codex shouldn't review anymore — or leave it for both.
8. `npm test` + `claude plugin validate $REPO` via Bash — must pass.
9. `claude plugin install cursor@cc-multi-cli-plugin --force` + `claude plugin install multi@cc-multi-cli-plugin --force`, via Bash.
10. Commit via Bash.
11. Tell the user: *"Done. Please restart Claude Code — plugin definitions are session-cached. After restart, try `/cursor:review` on a small diff."*
